...
@section('title', 'Edit Laporan Pelanggaran')
            ...
            <form action="{{ route('guru.laporan.update', $laporan->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="siswa_id" id="siswa_id" class="form-select @error('siswa_id') is-invalid @enderror" required>
                            ...
                            <option value="{{ $siswa->id }}" {{ old('siswa_id', $laporan->siswa_id) == $siswa->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', date('Y-m-d\TH:i', strtotime($laporan->tanggal))) }}" required>
                        @error('tanggal')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" id="jenis_pelanggaran_nama" readonly placeholder="-- Pilih Jenis Pelanggaran --" value="{{ old('jenis_pelanggaran_nama', $laporan->jenisPelanggaran->nama_pelanggaran) }}">
                        <input type="hidden" name="jenis_pelanggaran_id" id="jenis_pelanggaran_id" value="{{ old('jenis_pelanggaran_id', $laporan->jenis_pelanggaran_id) }}">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalJenisPelanggaran">
                            <i class="fa-solid fa-search"></i>
                        </button>
                    </div>
                    <small class="text-muted">Poin: <span id="poinDisplay">{{ $laporan->poin }}</span></small>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="4" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Jelaskan detail kejadian pelanggaran..." required>{{ old('keterangan', $laporan->keterangan) }}</textarea>
                    @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $laporan->foto_bukti) }}" alt="Foto Bukti" class="img-thumbnail" style="max-width: 200px;">
