...
@section('content')
    ...
    <div class="card orangtua-form-card shadow-sm mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 fw-bold">Edit: {{ $orangtua->user->nama_lengkap ?? 'N/A' }}</h6>
        </div>
        <div class="card-body p-4">
            
            <form action="{{ route('admin.orangtua.update', $orangtua) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_lengkap') is-invalid @enderror" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $orangtua->user->nama_lengkap) }}" required>
                            @error('nama_lengkap') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label for="username" class="form-label">Username <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('username') is-invalid @enderror" id="username" name="username" value="{{ old('username', $orangtua->user->username) }}" required>
                            @error('username') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label for="email" class="form-label">Email <span class="text-danger">*</span></label>
                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $orangtua->user->email) }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('siswa_id') is-invalid @enderror" id="siswa_id" name="siswa_id" required>
                                ...
                                @foreach($siswa as $s)
                                    <option value="{{ $s->id }}" {{ old('siswa_id', $orangtua->siswa_id) == $s->id ? 'selected' : '' }}>{{ $s->nama_siswa }} ({{ $s->nis }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('pendidikan') is-invalid @enderror" id="pendidikan" name="pendidikan">
                                ...
                                <option value="SD" {{ old('pendidikan', $orangtua->pendidikan) == 'SD' ? 'selected' : '' }}>SD</option>
                                <option value="SMP" {{ old('pendidikan', $orangtua->pendidikan) == 'SMP' ? 'selected' : '' }}>SMP</option>
                                <option value="SMA/SMK" {{ old('pendidikan', $orangtua->pendidikan) == 'SMA/SMK' ? 'selected' : '' }}>SMA/SMK</option>
                                <option value="D3" {{ old('pendidikan', $orangtua->pendidikan) == 'D3' ? 'selected' : '' }}>D3</option>
                                <option value="S1" {{ old('pendidikan', $orangtua->pendidikan) == 'S1' ? 'selected' : '' }}>S1</option>
                                <option value="S2" {{ old('pendidikan', $orangtua->pendidikan) == 'S2' ? 'selected' : '' }}>S2</option>
                                <option value="S3" {{ old('pendidikan', $orangtua->pendidikan) == 'S3' ? 'selected' : '' }}>S3</option>
                            </select>
