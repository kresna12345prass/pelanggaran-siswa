            ...
            <form action="{{ route('admin.aturan_pelanggaran.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('kategori_pelanggaran_id') is-invalid @enderror" id="kategori_pelanggaran_id" name="kategori_pelanggaran_id">
                        ...
                        @foreach($kategori as $k)
                            <option value="{{ $k->id }}" {{ old('kategori_pelanggaran_id') == $k->id ? 'selected' : '' }}>{{ $k->kategori_induk }} - {{ $k->nama_kategori }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('sanksi') is-invalid @enderror" id="sanksi" name="sanksi">
                        ...
                        @foreach($sanksi as $s)
                            <option value="{{ $s->nama_sanksi }}" {{ old('sanksi') == $s->nama_sanksi ? 'selected' : '' }}>
