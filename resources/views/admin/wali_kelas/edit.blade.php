...
@section('content')
    ...
    <div class="mb-4">
        <h1 class="h3 mb-1 text-gray-800">Edit Wali Kelas</h1>
        <p class="text-muted mb-0 small">Edit penugasan wali kelas</p>
    </div>
    ...
    <div class="card wali-kelas-form-card shadow-sm border-0">
        <div class="card-header py-3">
            <h6 class="m-0 fw-bold"><i class="fa-solid fa-pencil-alt me-2"></i>Form Edit Wali Kelas</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.wali_kelas.update', $waliKelas) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('tahun_ajaran_id') is-invalid @enderror" 
                                ...
                                @foreach($tahunAjaran as $ta)
                                    <option value="{{ $ta->id }}" 
                                            {{ (old('tahun_ajaran_id', $waliKelas->tahun_ajaran_id) == $ta->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('guru_id') is-invalid @enderror" 
                                ...
                                @foreach($guru as $g)
                                    <option value="{{ $g->id }}" 
                                            {{ (old('guru_id', $waliKelas->guru_id) == $g->id) ? 'selected' : '' }}>
                                        {{ $g->nama_guru }} ({{ $g->nip }})
                                    </option>
                                @endforeach
                            </select>
