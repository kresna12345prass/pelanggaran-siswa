...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0">✏️ Edit Pelanggaran</h3>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('kesiswaan.pelanggaran.update', $pelanggaran->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', \Carbon\Carbon::parse($pelanggaran->tanggal)->format('Y-m-d\TH:i')) }}" required>
                            @error('tanggal')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" id="jenis_pelanggaran_nama" readonly placeholder="-- Pilih Jenis Pelanggaran --" value="{{ $pelanggaran->jenisPelanggaran->nama_pelanggaran }}">
                        <input type="hidden" name="jenis_pelanggaran_id" id="jenis_pelanggaran_id" value="{{ old('jenis_pelanggaran_id', $pelanggaran->jenis_pelanggaran_id) }}">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalJenisPelanggaran">
                            <i class="fa-solid fa-search"></i>
                        </button>
                    </div>
                    <small class="text-muted">Poin: <span id="poinDisplay">{{ $pelanggaran->poin }}</span></small>
                ...
                <div class="mb-3">
                    ...
                    @if($pelanggaran->foto_bukti)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $pelanggaran->foto_bukti) }}" class="img-thumbnail" style="max-height: 150px" alt="Bukti">
                        </div>
                    @endif
